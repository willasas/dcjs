...
class dcCrypto {
  ...
  static async aesEncrypt(message, password) {
    ...
    try {
      ...
      // 加密数据
      const encrypted = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, messageData)
  ...
  static async hmacSha256(message, key) {
    ...
    const signature = await crypto.subtle.sign('HMAC', keyBuffer, encoder.encode(message))
  ...
  static async hashPassword(password) {
    ...
    const hashBuffer = await crypto.subtle.digest('SHA-256', new Uint8Array([...salt, ...passwordBuffer]))
    const hashArray = Array.from(new Uint8Array(hashBuffer))
