...
class dcEle {
  ...
  static removeClass(eleId, eleClassName) {
    ...
    elements.forEach(element => {
      if (element.classList) {
        ...
        element.className = element.className.replace(new RegExp('(^|\\b)' + eleClassName.split(' ').join('|') + '(\\b|$)', 'gi'), ' ').trim()
      }
    })
  }
  ...
  /**
   * 初始化标签切换功能，支持鼠标悬停和点击事件。
   *
   * @param {string} tabContainerSelector - 选择器字符串，用于选择包含标签的容器元素。
   * @param {string} contentContainerSelector - 选择器字符串，用于选择包含内容的容器元素。
   */
  static tabSwitcher = (tabContainerSelector, contentContainerSelector) => {
    /**
     * 通过选择器获取标签容器元素。
     * @type {HTMLElement}
     */
    ...
    if (!tabContainer || !contentContainer) {
      console.error('Element not found:', tabContainerSelector, contentContainerSelector)
      return
    }
     ...
     */
    const handleTabEnter = event => {
      if (event.target.tagName === 'LI') {
        console.log('Event:', event.type, 'Target:', event.target.textContent) // 调试信息
        const index = Array.from(tabContainer.children).indexOf(event.target)
     ...
     * @param {MouseEvent} event - 鼠标事件对象。
     */
    const handleTabLeave = event => {
      if (event.target.tagName === 'LI') {
        console.log('Event:', event.type, 'Target:', event.target.textContent) // 调试信息
        // 不在 mouseout 事件中清除 active 类
      }
    }
    ...
    // 绑定鼠标悬停事件
    tabContainer.addEventListener('mouseover', handleTabEnter)
    // 绑定鼠标离开事件
    tabContainer.addEventListener('mouseout', handleTabLeave)
    // 绑定点击事件
