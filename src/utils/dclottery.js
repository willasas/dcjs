...
class DCLottery {
  ...
  utils = {
    ...
    addRule(selector, styleObj, pos = 0) {
      try {
        ...
        // 构造完整的CSS规则
        const rule = selector.startsWith('@keyframes') ? `${selector} { ${cssText} }` : `${selector} { ${cssText} }`
    ...
    detectCSSPrefix() {
      ...
      for (const key in style) {
        const prefix = key.match(rxPrefixes) || (+key === +key && style[key].match(rxPrefixes))
    ...
    convertAllUnits(config) {
      // 定义需要转换单位的配置属性列表
      const numberProps = ['width', 'height', 'sbtnx', 'sbtny', 'sbtnw', 'sbtnh', 'boxw', 'boxh', 'bx', 'by', 'sx', 'sy', 'position']
      // 遍历配置属性列表，转换每个属性的单位
  ...
  startLottery(index) {
    ...
    if (this.disable()) {
      ...
      // 如果指定了索引，使用指定索引，否则随机
      const prizeIndex = typeof index === 'number' ? index : Math.floor(Math.random() * this.config.total)
