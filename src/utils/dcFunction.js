...
class dcFunction {
  ...
  static renderElement(config, container) {
    ...
    const element = typeof config.type === 'function' ? config.type(config.props) : document.createElement(config.type)

    // Set properties
    Object.entries(config.props).forEach(([key, value]) => {
      if (key === 'children') {
        value.forEach(child => {
          const childElement = child.type ? this.renderElement(child, element) : document.createTextNode(child.props.nodeValue)
  ...
  static timeout(fn, timeout) {
    return async function (...args) {
      return Promise.race([fn.apply(this, args), new Promise((_, reject) => setTimeout(() => reject(new Error('Function timeout')), timeout))])
    }
  }
  ...
  static overload() {
    ...
    return {
      ...
      implementation(...args) {
        const types = args.map(arg => (arg === null ? 'null' : Array.isArray(arg) ? 'array' : typeof arg)).join(',')
